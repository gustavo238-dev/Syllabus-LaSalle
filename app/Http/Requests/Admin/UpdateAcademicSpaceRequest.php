...
class UpdateAcademicSpaceRequest extends FormRequest
{
    ...
    {
        $academicSpaceId = $this->route('academicSpace')?->id;

        return [
            'competency_id' => ['required', 'integer', 'exists:competencies,id'],
            'name' => ['required', 'string', 'max:255'],
            'code' => ['required', 'string', 'max:20', "unique:academic_spaces,code,{$academicSpaceId}"],
