...
import type { BreadcrumbItem, Faculty, Program } from '@/types';
...
export default function ProgramsEdit({ program, faculties }: Props) {
    const breadcrumbs: BreadcrumbItem[] = [
        { title: 'Programas', href: ProgramController.index.url() },
        { title: program.name, href: ProgramController.edit.url(program) },
    ];

    return (
        <AdminLayout breadcrumbs={breadcrumbs}>
            <Head title={`Editar: ${program.name}`} />

            <div className="flex flex-1 flex-col gap-6 p-6">
                <PageHeader title={`Editar: ${program.name}`}>
                ...
                <Card className="max-w-2xl">
                    <CardContent className="pt-6">
                        <Form
                            {...ProgramController.update.form(program)}
                            className="space-y-5"
                        >
                            {({ errors, processing }) => (
                                <>
