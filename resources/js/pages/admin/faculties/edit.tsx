...
type Props = { faculty: Faculty };

export default function FacultiesEdit({ faculty }: Props) {
    const breadcrumbs: BreadcrumbItem[] = [
        { title: 'Facultades', href: FacultyController.index.url() },
        { title: faculty.name, href: FacultyController.edit.url(faculty) },
    ];

    return (
        <AdminLayout breadcrumbs={breadcrumbs}>
            <Head title={`Editar: ${faculty.name}`} />

            <div className="flex flex-1 flex-col gap-6 p-6">
                <PageHeader title={`Editar: ${faculty.name}`}>
